...
int Expression::nesting() { return nesting_; }

Expression::Expression(QWidget *parent) : Expression(nullptr, 1) {
  layout->setAlignment(Qt::AlignBottom | Qt::AlignRight);
}

Expression::Expression(Expression *parent, int nesting)
    : QWidget(nullptr), parent(parent), nesting_(nesting) {
  layout = new QGridLayout(this);
  layout->setSpacing(
      0); // Установка межколоночного и межстрочного пространства в 0
...
QString Expression::toString() {
  ...
  while (layout->itemAtPosition(1, i)) {
    if (layout->itemAtPosition(0, i - 1)) {
      Expression *expr = qobject_cast<Expression *>(
          layout->itemAtPosition(0, i - 1)->widget());
      str += "^{" + expr->toString() + "}";
    }
    if (layout->itemAtPosition(2, i - 1)) {
      Expression *expr = qobject_cast<Expression *>(
          layout->itemAtPosition(2, i - 1)->widget());
      str += "_{" + expr->toString() + "}";
    }
    EditableLabel *lexema =
        qobject_cast<EditableLabel *>(layout->itemAtPosition(1, i)->widget());
    str += "{" + lexema->text() + "}";
